export const EXPO_APP_CODE = `import React, { useState, useMemo } from 'react';
import {
  StyleSheet,
  Text,
  View,
...
export const EXPO_PACKAGE_JSON = `{
  "name": "aplikasi-konsumen-laporan",
  "version": "1.0.0",
  "main": "index.js",
  "scripts": {
    "start": "expo start",
...
export const EXPO_SETUP_STEPS = [
  {
    step: 1,
    title: 'Buat Project Expo Baru',
    command: 'npx create-expo-app@latest aplikasi-konsumen-laporan --template blank-typescript',
    desc: 'Buka terminal komputer Anda dan jalankan perintah ini untuk inisialisasi project Expo.',
  },
  {
    step: 2,
    title: 'Masuk ke Folder Project',
    command: 'cd aplikasi-konsumen-laporan',
    desc: 'Arahkan terminal ke dalam folder project yang baru dibuat.',
  },
  {
    step: 3,
    title: 'Install Library Excel & Sharing',
    command: 'npx expo install expo-file-system expo-sharing xlsx',
    desc: 'Library resmi Expo untuk menulis file Excel dan membagikannya ke WhatsApp/Google Drive.',
  },
  {
    step: 4,
    title: 'Salin Kode ke App.tsx',
    command: '// Salin kode di tab "App.tsx" ke file App.tsx pada project Anda',
    desc: 'Ganti seluruh isi file App.tsx dengan kode yang telah disiapkan.',
  },
  {
    step: 5,
    title: 'Jalankan di HP (Expo Go)',
    command: 'npx expo start',
    desc: 'Scan QR code di terminal menggunakan aplikasi Expo Go di HP Android atau iPhone Anda!',
  },
];

export const STREAMLIT_TO_EXPO_MAPPING = [
  {
    streamlitCode: 'col1.metric("Total Konsumen", total_konsumen)',
    expoEquivalent: '<View style={styles.metricCard}><Text style={styles.metricLabel}>Total Konsumen</Text><Text style={styles.metricValue}>{totalKonsumen}</Text></View>',
    feature: 'Metrik Ringkasan Total Konsumen',
  },
  {
    streamlitCode: 'col2.metric("Total Pinjaman", f"Rp {total_pinjaman:,.0f}")',
    expoEquivalent: '<View style={styles.metricCard}><Text style={styles.metricLabel}>Total Pinjaman</Text><Text style={styles.metricValue}>{formatRupiah(totalPinjaman)}</Text></View>',
    feature: 'Metrik Total Pinjaman (Format Rupiah IDR)',
  },
  {
    streamlitCode: 'st.bar_chart(grafik_data, x="Bulan", y="Pinjaman")',
    expoEquivalent: '<View style={styles.chartContainer}>{monthlyData.map(m => <View style={styles.barFill} />)}</View>',
    feature: 'Grafik Performa Pinjaman Bulanan (Bar Chart)',
  },
  {
    streamlitCode: 'with st.form("form_input"): st.text_input("Nama"), st.number_input("Pinjaman"), st.text_input("Nomor Kontrak")',
    expoEquivalent: '<TextInput value={nama} onChangeText={setNama} /> + <TextInput value={pinjaman} keyboardType="numeric" />',
    feature: 'Formulir Input Data Konsumen (7 Field)',
  },
  {
    streamlitCode: 'if nama and kontrak: st.success("Data berhasil disimpan!") else: st.error(...)',
    expoEquivalent: 'if (!nama.trim() || !kontrak.trim()) Alert.alert("Peringatan", ...) else Alert.alert("Sukses", ...)',
    feature: 'Validasi & Feedback Pengguna',
  },
  {
    streamlitCode: 'st.download_button("Unduh Laporan ke Excel", data=excel_data, file_name="laporan_konsumen.xlsx")',
    expoEquivalent: 'const base64 = XLSX.write(wb, { type: "base64" }); await FileSystem.writeAsStringAsync(...); await Sharing.shareAsync(fileUri);',
    feature: 'Ekspor Dokumen Excel (.xlsx) dengan Sheet "Laporan"',
  },
];
